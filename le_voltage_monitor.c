...
// IADC input GPIO port/pin configration  iadcPosInputPortCPin6 iadcPosInputPortCPin2
//  iadcPosInputPortCPin0 iadcPosInputPortDPin2
#define IADC_INPUT_POS            iadcPosInputPortCPin6
//#define IADC_INPUT_POS            iadcPosInputPortBPin4
...
void le_voltage_monitor_init(void)
{
  ...
  init_clocks();  // enables and configures all the clocks for the peripherals in this lab
  init_letimer();
  init_prs();
  init_iadc();
  init_ldma();
...
static void init_letimer(void)
{
  ...
  // The LETIMER0's timeout frequency determines the sampling frequency,
  // which can be modified with the #define SAMPLING_FREQ_HZ macro definition
  // in le_voltage_monitor.h.
...
static void init_prs(void)
{
  // PRS_SourceAsyncSignalSet() connects channel 1 of the producer-side of the PRS to the
  // LETIMER underflow event.
  ...
  // PRS_ConnectConsumer( ) function connects channel 1 of the consumer-side
  // of the PRS to the IADC start conversion trigger.
